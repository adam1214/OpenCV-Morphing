...
void morphTriangle(Mat & img1, Mat & img2, Mat & img, vector<Point2f> & t1, vector<Point2f> & t2, vector<Point2f> &t, float alpha) 
{
...
{
	/*
	arg #1: path to image 1
	arg #2: path to points for image 1
	arg #3: path to image 2
	arg #4: path to points for image 2
	arg #5: number of frames (min 2)
	arg #6: path to output results
	*/
	...
	imwrite("tri1.jpg", img1_copy);
	imwrite("tri2.jpg", img2_copy);
	...
	imwrite("result.jpg", img_res);
	Mat result_img = imread("result.jpg");
	namedWindow("Display morphing image", WINDOW_AUTOSIZE);
	imshow("Display morphing image", result_img);
	while (waitKey() != 27);
	return 0;
}
